...
AlarmMap alarmListMap[static_cast<int>(AlarmId::alarmNum)] = {
    {AlarmId::unkownId, AlarmType::other, AlarmPriority::unknowPriority},         {AlarmId::PHHigh, AlarmType::physiological, AlarmPriority::middlePriority},
    {AlarmId::PHLow, AlarmType::physiological, AlarmPriority::middlePriority},    {AlarmId::So2High, AlarmType::physiological, AlarmPriority::middlePriority},
    {AlarmId::So2Low, AlarmType::physiological, AlarmPriority::middlePriority},   {AlarmId::SPCO2High, AlarmType::physiological, AlarmPriority::middlePriority},
    {AlarmId::SPCO2Low, AlarmType::physiological, AlarmPriority::middlePriority}, {AlarmId::tempHigh, AlarmType::physiological, AlarmPriority::middlePriority},
...
bool AlarmDispatch::addAlarm(ALARM alarm)
{
    ...
    else    // the channel was insert before
    {
        ...
        {
            if (alarm.channel == (*iter).getChannel() && alarm.id == (*iter).getAlarmId())
            {
                break;
            }
        }
...
bool AlarmDispatch::removeAlarm(INT16 channel, AlarmId id, bool forceDelete)
{
    ...
    for (; iter != alarmsMap[channel].end(); ++iter)
    {
        if (channel == (*iter).getChannel() && id == (*iter).getAlarmId())
        {
            break;
        }
    }
...
void AlarmDispatch::getHighestAlarmPri(INT16& audioPri, INT16& msgPri, INT16& ledPri)
{
...
bool AlarmDispatch::getAlarmInfo(INT16 channel, AlarmId id, ALARM& alarm)
{
    if (!isChannelExisted(channel))
        return false;
...
void AlarmDispatch::removeMuteTechAlarm(INT16 channel, AlarmId id)
{
    ...
    {
        if (muteTechAlarmsVec[i].first == channel && muteTechAlarmsVec[i].second == id)
        {
...
void AlarmDispatch::clearAllMuteTechAlarms() { muteTechAlarmsVec.clear(); }
...
bool AlarmDispatch::isVisualOnlyAlarm(AlarmId alarmId) { return false; }


bool AlarmDispatch::setAcknowledged(bool ack)
{
    ALARM_ITER alarmIter = alarmsMap.begin();
    for (; alarmIter != alarmsMap.end(); ++alarmIter)
    {
...
bool AlarmDispatch::isPhysicalAlarmExisted() { return false; }
